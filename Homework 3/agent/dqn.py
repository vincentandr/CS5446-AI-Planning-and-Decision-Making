...
class ReplayBuffer():
    ...
    def push(self, transition):
        ...
        Output:
            * None
        '''
        # New element exceeding buffer limit automatically replaces element at index 0.
    ...
    def sample(self, batch_size):
        ...
        '''
        trans = np.random.choice(len(self.buffer), batch_size, replace=False)

        states, actions, rewards, next_states, dones = collections.deque(), collections.deque(), collections.deque(), collections.deque(), collections.deque()

        for i in trans:
            states.append(self.buffer[i].state)
            actions.append(self.buffer[i].action)
            rewards.append(self.buffer[i].reward)
            next_states.append(self.buffer[i].next_state)
            dones.append(self.buffer[i].done)

        t_states = torch.tensor(states, dtype=torch.float, device=device)
        t_actions = torch.tensor(actions, device=device)
        t_rewards = torch.tensor(rewards, dtype=torch.float, device=device)
        t_next_states = torch.tensor(next_states, dtype=torch.float, device=device)
        t_dones = torch.tensor(dones, dtype=torch.float, device=device)
...
class BaseAgent(Base):
    def act(self, state, epsilon=0.0):
        ...
        '''
        actions = self.forward(self, state)

        max_index = torch.argmax(actions, dim=1).item()

        length = list(actions[0].size())[0]
...
def compute_loss(model, target, states, actions, rewards, next_states, dones):
    t_target = torch.max(target(next_states), dim=1).values
    t_target[dones.squeeze() == 1.0] = 0
    t_target_res = rewards.squeeze() + gamma * t_target
    t_input = torch.gather(model(states), 1, actions)
    t_input = t_input.squeeze()

    loss = nn.SmoothL1Loss()

    res = loss(t_input, t_target_res)
    return res
