...
class GeneratePDDL_Stationary :
    ...
    def generateInitString(self) :
        ...
        for car in state.cars:
            car_string += "(blocked pt{}pt{})".format(car.position.x, car.position.y)
        ...
        for w in range(self.width):
            for lane in range(self.num_lanes):
                if lane == 0:
                    ...
                    down_next_y = lane + 1
                forward_next_y = lane
                next_string += "(up_next pt{}pt{} pt{}pt{})(down_next pt{}pt{} pt{}pt{})(forward_next pt{}pt{} pt{}pt{})".format(
                    w, lane, w-1, up_next_y, w, lane, w-1, down_next_y, w, lane, w-1, forward_next_y
                )

        init_string = car_string + agent_string + next_string
        return init_string
...
def generateDomainPDDLFile(gen):
    ...
    gen.addAction(name="UP",
                  parameters=(("ag", "agent"), ("from", "gridcell"), ("to", "gridcell")),
                  precondition_string="(and (not (blocked ?to)) (at ?from ?ag) (up_next ?from ?to))",
                  effect_string="(and (not (at ?from ?ag)) (at ?to ?ag))")
    gen.addAction(name="DOWN",
                  parameters=(("ag", "agent"), ("from", "gridcell"), ("to", "gridcell")),
                  precondition_string="(and (not (blocked ?to)) (down_next ?from ?to) (at ?from ?ag))",
                  effect_string="(and (not (at ?from ?ag)) (at ?to ?ag))")
    gen.addAction(name="FORWARD",
                  parameters=(("ag", "agent"), ("from", "gridcell"), ("to", "gridcell")),
                  precondition_string="(and (not (blocked ?to)) (forward_next ?from ?to) (at ?from ?ag))",
                  effect_string="(and (not (at ?from ?ag)) (at ?to ?ag))")
